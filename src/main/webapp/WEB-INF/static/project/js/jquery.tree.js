...
(function($) {
    ...
    $.fn.treeview = function(settings) {
        ...
        function buildtree(data, ht) {
            ...
            if (data && data.length > 0) {
                ...
                asnyloadc(null, false, function(data) {
                    ...
                    dfop.onload && $.isFunction(dfop.onload) && dfop.onload(data);
                });
            }
            ht.push("</ul>"); // root and;
        ...
        function builparam(node) {
            var p = [{ name: "nodeId", value: encodeURIComponent(node.id) }
                    , { name: "nodeText", value: encodeURIComponent(node.text) }
                    , { name: "nodeValue", value: encodeURIComponent(node.value) }]
            return p;
        }
        function bindevent() {
            $(this).hover(function() {
        ...
        function getck(items, c, fn) {
            for (var i = 0, l = items.length; i < l; i++) {
            	var result;
                (items[i].showcheck == true && (items[i].checkstate == 1 || items[i].checkstate == 2)) && (result=fn(items[i])) &&c.push(result);
        ...
        me[0].t = {
            ...
            getSelectedValues: function(fun) {
                var s = [];
                fun = fun || function(item) { return item.value; };
                getck(treenodes, s, fun);
                return s;
            },
    ...
    $.fn.getCheckedNodes = function(fun) {
        if (this[0].t) {
            return this[0].t.getSelectedValues(fun);
        }
        return null;
    };
